...
function appendResource(value) {
    ...
    var $div = $('<div class="result col-md-12"></div>');
    $('#results').append($div);

    // add three 'rows'
    var $rowA = $('<h4 class="rowA"></h4>').appendTo($div);
    var $rowB = $('<div class="rowB"></div>').appendTo($div);
    ...
    // row B
    $rowB.append('<div class="col-md-3"><span><strong class="resultsLabelFirst">'+jQuery.i18n.prop('datasets.js.appendtyperesource')+' : </strong>' + value.resourceType + '</span></div>');  // resource type
    $rowB.append('<div class="col-md-3"><span><strong class="resultsLabel">'+jQuery.i18n.prop('datasets.js.licence')+' : </strong>' + (value.licenseType == null ? '' : value.licenseType) + '</span></div>'); // license type
    $rowB.append('<div class="col-md-3"><span><strong class="resultsLabel">'+jQuery.i18n.prop('datasets.js.licence.version')+' : </strong>' + (value.licenseVersion == null ? '' : value.licenseVersion) + '</span></div>'); // license version
    if (value.resourceType == 'records') {
        $rowB.append('<div class="col-md-3"><span class="viewRecords"><a class="btn btn-default access-data" title="' + jQuery.i18n.prop('datasets.js.appendresource03') + '" href="' + biocacheUrl + '/occurrences/search?q=data_resource_uid:' + value.uid + '">'+jQuery.i18n.prop('datasets.js.view.records')+'</a></span></div>'); // records link
    }
    if (value.resourceType == 'website' && value.websiteUrl) {
        $rowB.append('<div class="col-md-4"><span class="viewWebsite"><a class="btn btn-default access-data" title="' + jQuery.i18n.prop('datasets.js.appendresource04') + '" target="_blank" href="' + value.websiteUrl + '">Website</a></span></div>'); // website link
    }
    ...
    if (value.contentTypes != null) {
        $rowC.append('<span><strong class="resultsLabel">' + jQuery.i18n.prop('datasets.js.contentinclude') + ' :</strong></span>'); // label for content types
...
function updateTotal() {
    ...
    $('#resultsReturned').html("<strong> " + total + " </strong>"+ (total == 1 ? jQuery.i18n.prop('public.datasets.resultsreturned.message02.sing') : jQuery.i18n.prop('public.datasets.resultsreturned.message02.plu')));
...
function showFilters() {
    ...
    if (currentFilters.length > 0) {
        ...
        $('#currentFilterHolder').append('<div id="currentFilter"><h4><span class="FieldName">' + jQuery.i18n.prop('datasets.js.current.filter') + '</span></h4>' +
    ...
    $.each(currentFilters, function(index, obj) {
        ...
        $('#currentFilter #subnavlist ul').append('<li>' + labelFor(obj.name) + ': <b>' + displayValue + '</b>&nbsp;' +
            '[<b><a href="#" onclick="removeFilter(\'' + addslashes(obj.name) + "','" + addslashes(obj.value) + '\',this);return false;"' +
...
function showPaginator() {
    ...
    var $pago = $("<div class='pagination button-int'></div>");
    // add prev
    if (offset > 0) {
        $pago.append('<a href="javascript:prevPage();" class="step">« Précédant</a></li>');
    }
    else {
        $pago.append('<span class="step">« Précédant</span>');
    }
    for (var i = 1; i <= maxPage && i<20; i++) {
        if (i == currentPage) {
            $pago.append('<span class="currentStep disabled">' + i + '</span>');
        }
        else {
            $pago.append('<a href="javascript:gotoPage(' + i + ');" class="step">' + i + '</a>');
        }
    }
    // add next
    if ((offset + pageSize()) < total) {
        $pago.append('<a href="javascript:nextPage();" class="step">Suivant »</a>');
    }
    else {
        $pago.append('<span class="step">Suivant »</span>');
    }
...
function moreLink() {
    var $more = $('<div class="button-ext"><div id="more" class="btn btn-default access-data button-int"><i class="icon-hand-right"></i>'+ jQuery.i18n.prop('datasets.js.show.more')+'</div></div>');
    $more.click(function() {
...
function lessLink() {
    var $less = $('<div class="button-ext"><div id="less" class="btn btn-default access-data button-int"><i class="icon-hand-right"></i> '+ jQuery.i18n.prop('datasets.js.show.less')+'</div></div>');
    $less.click(function() {
