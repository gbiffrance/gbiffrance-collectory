...
function dataRequestHandler(data) {
    ...
    switch (unMappable.length) {
        ...
        case 1: unMappedText = "Une " + jQuery.i18n.prop('map.js.collectioncannotbemapped'); break;
...
function updateList(features) {
    ...
    if (!$('div#all').hasClass('inst')) {  // don't change text if showing institutions
        $('span#allButtonTotal').html(jQuery.i18n.prop('show.all') + " " + maxCollections + " " + jQuery.i18n.prop('map.js.collections.filtre') + ".")
    }
    // update display of number of features
    var innerFeatures = "";
    switch (features.length) {
        ...
        default: innerFeatures = "Les "+ features.length + " " + jQuery.i18n.prop('map.js.collectionsarelistedalphabetically'); break;
    }
    ...
    for (var i = 0; i < sortedParents.length; i++) {
        ...
        for (var c = 0; c < collList.length; c++) {
            ...
            if (!coll.attributes.isMappable) {
                content += "<img style='vertical-align:middle; width:3%' src='" + baseUrl + "/images/map/nomap.gif'/>";
            }
            content += "</li>";
        }
        content += "</ul></li>"
...
function getSelectedFilters() {
    ...
    if (checked == 'fauna,entomology,microbes,plants,paleontology,fungis') {
        checked = 'all';
    }
