...
class CollectoryTagLib {
     ...
     */
    def helpTD = {
        // out << '<img class="helpButton" alt="help" src="' + resource(dir:'images/skin', file:'help.gif') + '" onclick="toggleHelp(this);"/>'
        // out << '<span class="glyphicon glyphicon-question-sign" aria-hidden="true" onclick="toggleHelp(this);"/>'
    }
...
//    }
    ...
    def checkboxSelect = {attrs ->
        ...
        attrs.from.eachWithIndex { it, index ->
            ...
            out << "<input name='${attrs.name}' type='checkbox' ${checked}' value='${it}'/>${it}"
            out << "</label>"
    ...
    def homeLink = {
        out << '<a class="home" href="' + createLink(uri:"/manage") + '">Administration</a>'
    }
    ...
    def editButton = { attrs, body ->
        // if (isAuthorisedToEdit(attrs.uid, request.getUserPrincipal()?.attributes?.email)) {
            def paramsMap
            // anchor class
            paramsMap = [class:'edit-btn btn btn-sm']
            // action
            ...
            // out << "<div class='btn btn-sm edit-btn'>"
            out << link(paramsMap) {body() ?: 'Editer'}
            // out << "</div>"
        // } else {
    ...
    def jsonSummaryLink = { attrs, body ->
        ...
        out << "<a class='json' href='${resource(dir:"lookup",file:"summary/${attrs.uid}.json")}'>Voir le résumé (JSON)</a>"
    }
    ...
    def jsonDataLink = { attrs, body ->
        ...
        out << "<a class='json' href='${uri}'> Voir les données (JSON)</a>"
    }
        ...
        if (license) {
            ...
            if(version.equalsIgnoreCase('4.0')){
                ...
                switch (license) {
                    case "CC BY":
                        image = "http://i.creativecommons.org/l/by/${version}/88x31.png";
                        link = "http://creativecommons.org/licenses/by/${version}/fr/"
                        break
                    case "CC BY-NC":
                        image = "http://i.creativecommons.org/l/by-nc/${version}/88x31.png";
                        link = "http://creativecommons.org/licenses/by-nc/${version}/fr/"
                        break
                    case "CC BY-SA":
                        image = "http://i.creativecommons.org/l/by-sa/${version}/88x31.png";
                        link = "http://creativecommons.org/licenses/by-sa/${version}/fr/"
                        break
                    case "CC BY-NC-SA":
                        image = "http://i.creativecommons.org/l/by-nc-sa/${version}/88x31.png";
                        link = "http://creativecommons.org/licenses/by-nc-sa/${version}/fr/"
                        break
                    default:
                        image = null
                        link = null
            ...
            if (attrs.imageOnly && image) {
                ...
                out << "${display.display?:'No licence specified'} ${attrs.version?:''} France (${license}) ${imageHtml}"
            }
            else {
                out << "${display.display?:'No licence specified'} ${attrs.version?:''} France (${license})"
            }
        }
    }
    ...
    def showConnectionParameters = { attrs ->
        ...
        if (attrs.connectionParameters?.toString()) {
            ...
            log.error("showConnectionParameters: 1 ")
